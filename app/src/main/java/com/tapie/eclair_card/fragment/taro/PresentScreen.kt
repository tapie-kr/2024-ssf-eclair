...
@Composable
fun PresentScreen(navController: NavController, sharedViewModel: SharedViewModel = viewModel()) {
    val presentCardNumber = sharedViewModel.selectedCards.value?.getOrNull(1)
    ...
    Row(
        ...
        Column(
            ...
            Column(
                ...
                if (presentCardNumber != null) {
                    DisplayCard(presentCardNumber, Modifier.size(230.dp, 380.dp))
                }
                Spacer(modifier = Modifier.height(8.dp)) // 빈 공간 조정

                Text(
                    text = "현재", // 선택된 카드에 따라 텍스트 변경 가능
